class_name WorldMap
extends Control
## Окно мировой карты: зоны, поход героя, армия на карте, гарнизоны, отчёты о боях.
...
func _update_zone_panel() -> void:
	...
	move_button.disabled = WorldService.is_marching() or not adjacent or enemy_castle
	if mine:
